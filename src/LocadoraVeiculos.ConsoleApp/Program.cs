...
namespace LocadoraVeiculos.ConsoleApp
{
    class MyClass
    {
        ...
        {
            var x = Convert.ToDecimal("1.100,00", new CultureInfo("pt-BR"));

            var parceiroId = AdicionarParceiro();

            var parceiro = SelecionarParceiro(parceiroId);

            AdicionarCupom(parceiro);
        ...
        private static int AdicionarParceiro()
        {
            ...
            //IParceiroRepository repository = new ParceiroOrmDao(db);

            //repository.Inserir(parceiro);
